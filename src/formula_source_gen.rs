...
pub(crate) fn generate_z3_source(z3type: Z3Type, e: &Expr) -> String {
    ...
    let binary = |op_str: &str, lhs: &Expr, rhs: &Expr| -> String {
        format!(
            ...
            z3type.z3_type_name(),
            op_str,
            &generate_z3_source(z3type, lhs),
            &generate_z3_source(z3type, rhs)
        )
    };
    ...
    match e {
        // 42
        Expr::Lit(l) => match z3type {
            Z3Type::Int => {
                format!("&z3::ast::Int::from_i64(&ctx, {})", l.to_token_stream())
            }
            Z3Type::BV(width) => {
                format!(
                    "&z3::ast::BV::from_u64(&ctx, {} as u64,{})",
                    l.to_token_stream(),
                    width
                )
            }
        ...
        Expr::Binary(e) => match e.op {
            BinOp::Add(_) => match z3type {
                Z3Type::Int => binary_lst("add", &e.left, &e.right),
                Z3Type::BV(_width) => binary_noctx("bvadd", &e.left, &e.right),
            },
            BinOp::Sub(_) => binary("sub", &e.left, &e.right),
            BinOp::Mul(_) => binary_lst("mul", &e.left, &e.right),
            BinOp::Div(_) => binary("div", &e.left, &e.right),
            BinOp::And(_) => {
                format!(
                    "&z3::ast::Bool::and(&ctx, &[{}, {}])",
                    &generate_z3_source(z3type, &e.left),
                    &generate_z3_source(z3type, &e.right)
                )
            }
            BinOp::Or(_) => {
                format!(
            ...
            BinOp::Eq(_) => binary_noctx("_eq", &e.left, &e.right),
            BinOp::Lt(_) => binary_noctx("lt", &e.left, &e.right),
            BinOp::Le(_) => binary_noctx("le", &e.left, &e.right),
            BinOp::Ne(_) => {
                format!(
                    "&z3::ast::Bool::not(&{})",
            ...
            BinOp::Ge(_) => binary_noctx("ge", &e.left, &e.right),
            BinOp::Gt(_) => binary_noctx("gt", &e.left, &e.right),
            BinOp::BitXor(_) => binary_bvonly("bvxor", &e.left, &e.right),
            BinOp::BitAnd(_) => binary_bvonly("bvand", &e.left, &e.right),
            BinOp::BitOr(_) => binary_bvonly("bvor", &e.left, &e.right),
            BinOp::Shl(_) => binary_bvonly("bvshl", &e.left, &e.right),
            BinOp::Shr(_) => binary_bvonly("bvshr", &e.left, &e.right),
